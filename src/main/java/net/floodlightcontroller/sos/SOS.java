...
public class SOS implements IOFMessageListener, IOFSwitchListener, IFloodlightModule  {
	...
	public void startUp(FloodlightModuleContext context) {
		...
		try {
			...
			SRC_AGENT_OVS_PORT = Short.parseShort(configOptions.get("src-agent-ovs-port"));
			...
			DST_AGENT_OVS_PORT = Short.parseShort(configOptions.get("src-agent-ovs-port"));
	...
	public boolean isCallbackOrderingPrereq(OFType type, String name) {
		...
		if (type == OFType.PACKET_IN && name.equals(DeviceManagerImpl.class.getSimpleName())) {
			return true;
		} else {
			return false;
		}
	...
	public void pushSOSFlow_2(SOSConnection conn) {
		...
		match.setInputPort(SRC_AGENT_OVS_PORT);
		match.setDataLayerType(Ethernet.TYPE_IPv4);
		match.setNetworkSource(conn.getSrcClient().getIPAddr());
		match.setNetworkProtocol(IPv4.PROTOCOL_TCP);
		match.setTransportSource(conn.getSrcPort());
		...
		flow.setPriority((short) 32768);
		flow.setIdleTimeout(FLOW_TIMEOUT);
		flow.setLengthU(OFFlowMod.MINIMUM_LENGTH + dldestAction.getLengthU() + tpdestAction.getLengthU() + nwdestAction.getLengthU() 
	...
	public void pushSOSFlow_3(SOSConnection conn) {
		...
		outputAction.setType(OFActionType.OUTPUT);
		outputAction.setPort(SRC_AGENT_OVS_PORT);
		outputAction.setLength((short) OFActionOutput.MINIMUM_LENGTH);
		actionList.add(outputAction);

		flow.setCookie(0);
		flow.setBufferId(-1);
		flow.setOutPort(SRC_AGENT_OVS_PORT);
		flow.setActions(actionList);
		flow.setMatch(match);
		flow.setPriority((short) 32768);
		flow.setIdleTimeout(FLOW_TIMEOUT);
		flow.setLengthU(OFFlowMod.MINIMUM_LENGTH + dlsrcAction.getLengthU() + tpsrcAction.getLengthU() + nwsrcAction.getLengthU() 
				+ outputAction.getLengthU());
	...
	public void pushSOSFlow_6a(SOSConnection conn) {
		...
		outputAction.setType(OFActionType.OUTPUT);
		outputAction.setPort(SRC_NTWK_PORT);
		outputAction.setLength((short) OFActionOutput.MINIMUM_LENGTH);
		actionList.add(outputAction);

		flow.setCookie(0);
		flow.setBufferId(-1);
		flow.setOutPort(SRC_NTWK_PORT);
		flow.setActions(actionList);
		flow.setMatch(match);
		flow.setPriority((short) 32768);
		flow.setIdleTimeout(FLOW_TIMEOUT);
		...
		String flowName = "sos-6a-" + IPv4.fromIPv4Address(conn.getSrcClient().getIPAddr()) + 
				"-" + portToString(conn.getSrcPort()) + 
				"-to-" + IPv4.fromIPv4Address(conn.getDstClient().getIPAddr()) +
				"-" + portToString(conn.getDstPort());
		sfp.addFlow(flowName, flow, conn.getSrcNtwkSwitch().getSwitch().getStringId());
	...
	public void pushSOSFlow_6b(SOSConnection conn) {
		...
		match.setInputPort(DST_NTWK_PORT);
		match.setDataLayerType(Ethernet.TYPE_IPv4);
		match.setNetworkDestination(conn.getDstAgent().getIPAddr());
		match.setNetworkProtocol(IPv4.PROTOCOL_TCP);
		// Don't care about the TCP port number
		...
		String flowName = "sos-6b-" + IPv4.fromIPv4Address(conn.getSrcClient().getIPAddr()) + 
				"-" + portToString(conn.getSrcPort()) + 
				"-to-" + IPv4.fromIPv4Address(conn.getDstClient().getIPAddr()) +
				"-" + portToString(conn.getDstPort());
	...
	public void pushSOSFlow_7(SOSConnection conn) {
		...
		match.setInputPort(DST_AGENT_OVS_PORT);
		match.setDataLayerType(Ethernet.TYPE_IPv4);
	...
	public void pushSOSFlow_8(SOSConnection conn) {
		...
		outputAction.setType(OFActionType.OUTPUT);
		outputAction.setPort(DST_AGENT_OVS_PORT);
		outputAction.setLength((short) OFActionOutput.MINIMUM_LENGTH);
		actionList.add(outputAction);

		flow.setCookie(0);
		flow.setBufferId(-1);
		flow.setOutPort(DST_AGENT_OVS_PORT);
		flow.setActions(actionList);
		flow.setMatch(match);
		flow.setPriority((short) 32768);
		flow.setIdleTimeout(FLOW_TIMEOUT);
	...
	public void pushSOSFlow_9a(SOSConnection conn) {
		...
		outputAction.setType(OFActionType.OUTPUT);
		outputAction.setPort(DST_NTWK_PORT);
		outputAction.setLength((short) OFActionOutput.MINIMUM_LENGTH);
		actionList.add(outputAction);

		flow.setCookie(0);
		flow.setBufferId(-1);
		flow.setOutPort(DST_NTWK_PORT);
		flow.setActions(actionList);
		flow.setMatch(match);
		flow.setPriority((short) 32768);
		flow.setIdleTimeout(FLOW_TIMEOUT);
		...
		String flowName = "sos-9a-" + IPv4.fromIPv4Address(conn.getSrcClient().getIPAddr()) + 
				"-" + portToString(conn.getSrcPort()) + 
				"-to-" + IPv4.fromIPv4Address(conn.getDstClient().getIPAddr()) +
				"-" + portToString(conn.getDstPort());
		sfp.addFlow(flowName, flow, conn.getDstNtwkSwitch().getSwitch().getStringId());
	...
	public void pushSOSFlow_9b(SOSConnection conn) {
		...
		match.setInputPort(SRC_NTWK_PORT);
		match.setDataLayerType(Ethernet.TYPE_IPv4);
		match.setNetworkDestination(conn.getSrcAgent().getIPAddr());
		match.setNetworkProtocol(IPv4.PROTOCOL_TCP);
		// Don't care about the TCP port number
		...
		String flowName = "sos-9b-" + IPv4.fromIPv4Address(conn.getSrcClient().getIPAddr()) + 
				"-" + portToString(conn.getSrcPort()) + 
				"-to-" + IPv4.fromIPv4Address(conn.getDstClient().getIPAddr()) +
				"-" + portToString(conn.getDstPort());
		sfp.addFlow(flowName, flow, conn.getSrcNtwkSwitch().getSwitch().getStringId());
